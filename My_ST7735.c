...
void My_GPIO_Init(void) {
    ...
    // C?u hình PA5 (SCK) và PA7 (MOSI) làm chân Alternate Function Push-Pull
    GPIOA->CRL &= ~(GPIO_CRL_MODE5 | GPIO_CRL_CNF5 | GPIO_CRL_MODE7 | GPIO_CRL_CNF7);
    GPIOA->CRL |= (GPIO_CRL_MODE5_1 | GPIO_CRL_MODE5_0 | GPIO_CRL_CNF5_1) | // PA5: 50MHz, AF_PP
                  (GPIO_CRL_MODE7_1 | GPIO_CRL_MODE7_0 | GPIO_CRL_CNF7_1);   // PA7: 50MHz, AF_PP

    // C?u hình PB0 (CS), PB1 (DC), PB3 (RST) làm Output Push-Pull
    GPIOB->CRL &= ~(GPIO_CRL_MODE0 | GPIO_CRL_CNF0 | GPIO_CRL_MODE1 | GPIO_CRL_CNF1 | GPIO_CRL_MODE3 | GPIO_CRL_CNF3);
    GPIOB->CRL |= (GPIO_CRL_MODE0_1 | GPIO_CRL_MODE0_0) | // PB0: 50MHz, Push-Pull
                  (GPIO_CRL_MODE1_1 | GPIO_CRL_MODE1_0) | // PB1: 50MHz, Push-Pull
                  (GPIO_CRL_MODE3_1 | GPIO_CRL_MODE3_0);  // PB3: 50MHz, Push-Pull

    // Ð?t tr?ng thái ban d?u cho các chân PB0, PB1, PB3
    GPIOB->ODR |= GPIO_ODR_ODR0; // CS ban d?u ? m?c cao
    GPIOB->ODR |= GPIO_ODR_ODR1; // DC ban d?u ? m?c cao
    GPIOB->ODR |= GPIO_ODR_ODR3; // RST ban d?u ? m?c cao
}
void SPI1_Init(void) {
    // Kích ho?t clock cho SPI1 và GPIOA
    RCC->APB2ENR |= RCC_APB2ENR_SPI1EN;

    // C?u hình SPI1
    SPI1->CR1 = 0; // Xóa t?t c? c?u hình ban d?u
    SPI1->CR1 |= SPI_CR1_MSTR;                // Ch? d? Master
    SPI1->CR1 |= SPI_CR1_BR_0;                // T?c d? SCK: F_PCLK/4 (tuong duong ~9 MHz)
    SPI1->CR1 &= ~SPI_CR1_CPOL;               // CPOL = 0
    SPI1->CR1 &= ~SPI_CR1_CPHA;               // CPHA = 0
    SPI1->CR1 |= SPI_CR1_SSM | SPI_CR1_SSI;   // Kích ho?t Software Slave Management
    SPI1->CR1 |= SPI_CR1_SPE;                 // Kích ho?t SPI
}
void SPI1_Transmit(uint8_t data) {
    while (!(SPI1->SR & SPI_SR_TXE)); // Ch? TXE = 1 (buffer tr?ng)
    SPI1->DR = data;                  // G?i d? li?u
    while (SPI1->SR & SPI_SR_BSY);    // Ch? BSY = 0 (SPI không b?n)
}
void ST7735_WriteCommand(uint8_t cmd) {
    GPIOB->ODR &= ~GPIO_ODR_ODR1; // DC = 0 (Command)
    GPIOB->ODR &= ~GPIO_ODR_ODR0; // CS = 0 (Ch?n màn hình)
    SPI1_Transmit(cmd);           // G?i l?nh
    GPIOB->ODR |= GPIO_ODR_ODR0;  // CS = 1 (B? ch?n màn hình)
}

void ST7735_WriteData(uint8_t data) {
    GPIOB->ODR |= GPIO_ODR_ODR1;  // DC = 1 (Data)
    GPIOB->ODR &= ~GPIO_ODR_ODR0; // CS = 0 (Ch?n màn hình)
    SPI1_Transmit(data);          // G?i d? li?u
    GPIOB->ODR |= GPIO_ODR_ODR0;  // CS = 1 (B? ch?n màn hình)
}
void ST7735_Reset(void) {
    GPIOB->ODR |= GPIO_ODR_ODR3;  // RST = 1
    for (volatile int i = 0; i < 100000; i++); // Delay
    GPIOB->ODR &= ~GPIO_ODR_ODR3; // RST = 0
    for (volatile int i = 0; i < 100000; i++); // Delay
    GPIOB->ODR |= GPIO_ODR_ODR3;  // RST = 1
    for (volatile int i = 0; i < 100000; i++); // Delay
}
...
// Rather than a bazillion ST7735_WriteCommand() and ST7735_WriteData() calls, screen
...
void ST7735_CommonInit(const uint8_t *cmdList) {
    ST7735_Reset(); // Reset ph?n c?ng màn hình

    // N?u có danh sách l?nh, g?i t?ng l?nh
    if (cmdList) {
        while (1) {
            uint8_t cmd = *cmdList++;
            if (cmd == 0xFF) break; // K?t thúc danh sách
            uint8_t numArgs = *cmdList++;
...
void ST7735_InitB(void) {
    ST7735_CommonInit(Bcmd); // G?i l?nh kh?i t?o
...
void static setAddrWindow(uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1) {
  ...
  ST7735_WriteData(x0+ColStart);     // XSTART
  ST7735_WriteData(0x00);
  ST7735_WriteData(x1+ColStart);     // XEND

  ST7735_WriteCommand(ST7735_RASET); // Row addr set
  ST7735_WriteData(0x00);
  ST7735_WriteData(y0+RowStart);     // YSTART
  ST7735_WriteData(0x00);
  ST7735_WriteData(y1+RowStart);     // YEND
...
void static pushColor(uint16_t color) {
  ST7735_WriteData((uint8_t)(color >> 8));
  ST7735_WriteData((uint8_t)color);
}
...
void ST7735_DrawBitmap(int16_t x, int16_t y, const uint16_t *image, int16_t w, int16_t h){
  ...
  for(y=0; y<h; y=y+1){
    for(x=0; x<w; x=x+1){
                                        // send the top 8 bits
      ST7735_WriteData((uint8_t)(image[i] >> 8));
                                        // send the bottom 8 bits
      ST7735_WriteData((uint8_t)image[i]);
...
void ST7735_SetRotation(uint8_t m) {
  ...
  switch (Rotation) {
   case 0:
     if (TabColor == INITR_BLACKTAB) {
       ...
       ST7735_WriteData(MADCTL_MX | MADCTL_MY | MADCTL_BGR);
     }
   ...
   case 1:
     if (TabColor == INITR_BLACKTAB) {
       ...
       ST7735_WriteData(MADCTL_MY | MADCTL_MV | MADCTL_BGR);
     }
  ...
  case 2:
     if (TabColor == INITR_BLACKTAB) {
       ST7735_WriteData(MADCTL_RGB);
     } else {
       ST7735_WriteData(MADCTL_BGR);
     }
     _width  = ST7735_TFTWIDTH;
     _height = ST7735_TFTHEIGHT;
    break;
   case 3:
     if (TabColor == INITR_BLACKTAB) {
       ST7735_WriteData(MADCTL_MX | MADCTL_MV | MADCTL_RGB);
     } else {
       ST7735_WriteData(MADCTL_MX | MADCTL_MV | MADCTL_BGR);
     }
     _width  = ST7735_TFTHEIGHT;
     _height = ST7735_TFTWIDTH;
     break;
